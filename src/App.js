...
function App() {
  ...
  const filterItems = (category) => { if (category === "all") {
    setDailyNews(items);
      return;
    }
  ...
  const filterAuthor = (author) => { if (author === "all") {
    setDailyNews(items);
      return;
    }
  ...
  const filterDate = (date) => { if (date === "all") {
    setDailyNews(items);
      return;
    }
 ...
 const paginate = pageNumber => setCurrentPage(pageNumber);
  ...
  <div className="pagination_filter">    
   <div className="pagination_move"> <Pagination  postsPerPage={postPerPage}  totalPosts={items.length}   paginate={paginate} /></div>
    <div className="dropdown">
     
  <button className="btn btn-secondary dropdown-toggle dropdown_filter" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
   Filter
  </button>
  <ul className="dropdown-menu" aria-labelledby="dropdownMenuButton1">
  <li className="filters_heading">Games</li>
  <li className="filter_parts"><Categories categories={categories} filterItems={filterItems} /></li>
  <li  className="filters_heading">Authors</li>
  <li className="filter_parts"> <Author categories={author} filterItems={filterAuthor} /></li>
  <li className="filters_heading">Date</li>
  <li className="filter_parts"><Date categories={date} filterItems={filterDate} /></li>

  </ul>
</div>
</div> 
        <div className="filter_all_option">
